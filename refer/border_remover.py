...
class BorderRemover:
    ...
    def _detect_corner_borders(self, binary: np.ndarray, h: int, w: int) -> np.ndarray:
        ...
        protected_corners = []
        if self.preserve_timing_marks:
            protected_corners = [1, 3]  # 右上角和右下角

        # 定义四个角落的种子点（从边缘向内几个像素）
        seed_points = []

        # 左上角
        if 0 not in protected_corners:
            seed_points.append((1, 1))

        # 右上角
        if 1 not in protected_corners:
            seed_points.append((w - 2, 1))

        # 左下角
        if 2 not in protected_corners:
            seed_points.append((1, h - 2))

        # 右下角
        if 3 not in protected_corners:
            seed_points.append((w - 2, h - 2))

        # 对每个种子点进行漫水填充
        for seed in seed_points:
            # 检查种子点是否为白色（在二值化图像中，白色=255对应原图黑色）
            if binary_copy[seed[1], seed[0]] == 255:
                ...
                # 将填充结果添加到掩膜（去掉padding）
                corner_mask = cv2.bitwise_or(corner_mask, mask_floodfill[1:h+1, 1:w+1])
